    ...
    while (this.pos < this.input.length) {
      ...
      if (ch === '!') {
        ...
        if (this.peek() === '=') {
          ...
          throw new FilterParserError({
            message: `Unexpected character '!' at position ${startPos}. Did you mean '!='?`,
            stage: 'tokenizer',
            position: startPos,
            length: 1,
            snippet: '!',
...
class SQLTranslator {
  ...
  private translateDenormalizedField(
    ...
    if (type === 'array' && operator !== 'CONTAINS') {
      throw new FilterParserError({
        message: `Operator ${operator} not supported for array field ${fieldName}. Use CONTAINS instead.`,
        stage: 'translator',
        position: 0,
        snippet: `@metadata.${fieldName} ${operator}`,
