...
export function loadRefinementConfig(): RefinementConfig {
  ...
  const accessTrackingTopN = Math.max(1, Number(process.env.MEMORY_ACCESS_TRACKING_TOP_N ?? 3)) || 3;

  const boostEnv = process.env.MEMORY_ACCESS_PRIORITY_BOOST;
  const parsedBoost = boostEnv ? Number(boostEnv) : 0.01;
  const accessPriorityBoost = Math.max(0, Math.min(1, Number.isFinite(parsedBoost) ? parsedBoost : 0.01));

  const queryExpansionEnabled = boolFromEnv(process.env.MEMORY_QUERY_EXPANSION_ENABLED, true);
  const queryExpansionCount = Math.max(1, Math.min(3, Number(process.env.MEMORY_QUERY_EXPANSION_COUNT ?? 2) || 2));
